(function ($) {
  ...
  window.ChargXApplePayHandler = {
    initialized: false,

    init: function () {
      if (window.ChargXApplePayHandler.initialized) {
        return;
      }

      window.ChargXApplePayHandler.initialized = true;

      $(document.body).on(
        "click",
        "#chargx-applepay-button",
        window.ChargXApplePayHandler.onApplePayClick
      );
    ...
    onApplePayClick: function (e) {
      ...
      fetch("https://api.chargx.io/pretransact", {
        ...
        .then(function (data) {
          ...
          window.ChargXApplePayHandler.startApplePaySession(pr);
        })
        .catch(function (err) {
